...
paths = [
    Path("config/quickshell/awtarchy/LockscreenPresentationState.js"),
    Path("config/quickshell/awtarchy-lock/LockscreenPresentationState.js"),
]
original = paths[0].read_text(encoding="utf-8")
if paths[1].read_text(encoding="utf-8") != original:
    raise SystemExit("presentation-state copies diverged before Task 2")
...
show_label_replacement = (
    '        normalized.format = String(raw.format || "24h").toLowerCase() === "12h" ? "12h" : "24h";\n'
    '        normalized.show_label = raw.show_label !== false;\n'
    '        normalized.visible = raw.visible !== false;'
)
if 'normalized.show_label = raw.show_label !== false;' not in text:
    if show_label_anchor not in text:
        raise SystemExit("timezone show_label anchor missing")
    text = text.replace(show_label_anchor, show_label_replacement, 1)

if 'function sharedProfile(state)' not in text:
    text += r'''
