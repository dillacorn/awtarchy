-- ~/.config/hypr/hyprlang.lua
-- Hyprlang-style compatibility helpers for the cleaned Hyprland Lua config.
...
-- Short bind helpers used by hyprland.lua.
-- b()    = bind
-- be()   = binde / repeat bind
-- bel()  = bindel
-- bl()   = bindl
-- bm()   = bindm / mouse bind
function b(mods, key, dispatcher, args)
    bind_old("bind", mods, key, dispatcher, args or "")
end

function be(mods, key, dispatcher, args)
    bind_old("binde", mods, key, dispatcher, args or "")
end

function bel(mods, key, dispatcher, args)
    bind_old("bindel", mods, key, dispatcher, args or "")
end

function bl(mods, key, dispatcher, args)
    bind_old("bindl", mods, key, dispatcher, args or "")
end

function bm(mods, key, dispatcher, args)
    bind_old("bindm", mods, key, dispatcher, args or "")
end

-- Clean helper aliases for rewritten hyprland.lua.
-- The *_old functions remain the compatibility parser layer.
cfg = cfg or config_set
var = var or set_var
