...
local ROOT = "awt-bookmarks://collection/"
...
local function state_file() return state_dir() .. "/awtarchy-bookmarks.txt" end
...
local subscribe = ya.sync(function(self)
    ...
    ps.sub_remote(KIND, function(incoming)
        ...
        write_state(self.bookmarks)
        ps.pub(KIND, self.bookmarks)
        if tostring(cx.active.current.cwd):match("^awt%-bookmarks://") then ya.emit("refresh", {}) end
    end)
end)
