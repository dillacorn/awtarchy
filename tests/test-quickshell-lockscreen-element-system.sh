...
custom_images="$(jq -cn --arg path "$image_path" '[{
    id:"image-test_1", path:$path, x:0.42, y:0.46, scale:50,
...
jq -e --arg expected "$(readlink -f -- "$image_path")" '
    .lockscreen_layout.logo.scale == 6
    and .lockscreen_layout.logo.stretch_x == 1.5
    ...
    and .lockscreen_custom_images[0].scale == 50
...
overscale_images="$(jq -cn --arg path "$image_path" '[{
    id:"image-too-large",path:$path,x:0.5,y:0.5,scale:100.01,
    stretch_x:1,stretch_y:1,opacity:100,visible:true
}]')"
if run_state save-lockscreen-editor \
    "$expanded_layout" "$valid_visibility" black '#000000' '' \
    cover 0.5 0.5 none 0 0 auto "$overscale_images" >/dev/null 2>&1; then
    fail 'custom image scale above defensive 100x bound was accepted'
fi

overscale_layout="${expanded_layout/\"scale\":6/\"scale\":100.01}"
if run_state save-lockscreen-layout "$overscale_layout" >/dev/null 2>&1; then
    fail 'built-in element scale above defensive 100x bound was accepted'
fi
...
require_text "$BAR_STATE" 'scale > 100.00' \
    'BarState does not use the common defensive scale bound'
...
require_text "$SCENE" 'const maximum = 100.00;' \
    'renderer still has a special 200%/10x scale ceiling'
...
require_text "$EDITOR" 'readonly property real elementScaleMaximum: 100.0' \
    'editor does not expose one high defensive scale bound'
