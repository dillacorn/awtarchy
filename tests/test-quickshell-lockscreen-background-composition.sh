...
require_text "$STATE" 'lockscreen_wallpaper_blur: 10' 'state default lacks 10% wallpaper blur'
require_text "$STATE" 'lockscreen_blur_style: "pixelated"' 'state default lacks pixelated blur style'
...
require_text "$STATE" 'normalize_percent_integer()' 'overlay percentage has no validator'
require_text "$STATE" 'normalize_lockscreen_blur_integer()' '0-200 blur strength has no dedicated validator'
...
require_text "$STATE" 'local wallpaper_blur="${11:-10}"' 'editor save does not accept wallpaper blur'
require_text "$STATE" 'local blur_style="${19:-pixelated}"' 'editor save does not accept blur style'
...
require_text "$BAR" 'lockscreen_wallpaper_blur: 10' 'BarState default lacks 10% wallpaper blur'
...
require_text "$EDITOR" 'property int draftWallpaperBlur: 10' 'editor has no 10% blur draft'
require_text "$EDITOR" 'property string draftBlurStyle: "pixelated"' 'editor has no pixelated blur-style draft'
