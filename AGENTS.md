...
- Keep Yazi Enter context-aware: Enter descends into a highlighted directory and otherwise opens the highlighted file normally. Preserve normal single-click selection; a second left-click on the already highlighted directory enters it. Keep Yazi's existing right-click open behavior and use the stable `Entity:click()` / `cha.is_dir` API rather than external plugins.
